#*** NOTE forked by Jeff Muday, Wake Forest Univ. 2019
#
#
...
def run_inference_on_batch(batch, num_predictions=5, verbose=True):
    """Runs inference on an image.

    Args:
      batch: a list of Image file names.
      num_predictions: how many predictions will be returned
      verbose: echos data to the console
    ...
    with tf.Session() as sess:
        ...
        for image in batch:
            if not tf.gfile.Exists(image):
                tf.logging.fatal('File does not exist <%s> skipping', image)
                continue
            ...
            for node_id in top_k:
                ...
                if verbose:
                    print('%s (score = %.5f)' % (human_string, score))
                inf_data.append({'score':score,'ident':human_string})
            batch_result.append(inf_data)

        return batch_result

def classify_batch(image_dir='./images',results_file='results.csv', verbose=True, batch_size=10, num_predictions=1):
    """
    Top-level image directory batch classifier.

    Calls the "run_inference_on_batch()" to classify a "batch" of files with the Inception3 classifier.  See comments on run_inference_on_batch()
    for further details.

    Since the "batch" or images could be in the thousands, we may have a limited amount of memory available.
    Thus, the function will break these down into smaller sets of files whose size is governed by the batch_size parameter.
    """
    # write results file header
    with open(results_file,'w') as fh:
        fh.write('id,species')
        for i in range(num_predictions):
            fh.write(',score{},ident{}'.format(i,i))
        fh.write('\n')

    # get the model and labels for inception3
    maybe_download_and_extract()
    ...
    while idx < len(dlist):
        ...
        batch_files = []
        while (len(batch_files) < batch_size) and (idx < len(dlist)):
            fname = dlist[idx]
            if 'jpg' in fname.lower():
                batch_files.append(os.path.join(image_dir, fname))
            idx += 1

        # attempt classification on the batch_files
        batch_data = run_inference_on_batch(batch_files, num_predictions=num_predictions, verbose=False)

        if verbose:
            for i, fname in enumerate(batch_files):
                bi = idx-len(batch_files) + i
                print("{}. file: {}".format(bi, fname))
                for data in batch_data[i]:
                    print('score:{}, id:{}'.format(data.get('score'), data.get('ident') ))

        if len(batch_data) > 0:
            for i, fname in enumerate(batch_files):
                bi = idx-len(batch_files) + i
                with open(results_file, 'a') as fh:
                    fh.write('{},{}'.format(bi, fname))
                    for data in batch_data[i]:
                        fh.write(',"{}",{}'.format(data.get('ident'), data.get('score')))
                    fh.write('\n')
